...
var pencil = (function() {
	...
	function buildPencil() {
		...
		var body = new THREE.Mesh(
			...
			new THREE.MeshLambertMaterial({ color: 0xf5b301 }));
		group.add(body);
		var stripe = new THREE.Mesh(
			new THREE.CylinderGeometry(0.505, 0.505, 5.7, 6, 1, true, 0, Math.PI / 3),
			new THREE.MeshLambertMaterial({ color: 0x222222 }));
		group.add(stripe);
		var wood = new THREE.Mesh(
			new THREE.ConeGeometry(0.5, 1.3, 6),
			new THREE.MeshLambertMaterial({ color: 0xe8c39e }));
		wood.position.y = -3.65; wood.rotation.x = Math.PI;
		group.add(wood);
		var lead = new THREE.Mesh(
			new THREE.ConeGeometry(0.16, 0.55, 8),
			new THREE.MeshLambertMaterial({ color: 0x2b2b2b }));
		lead.position.y = -4.5; lead.rotation.x = Math.PI;
		group.add(lead);
		var holder = new THREE.Mesh(
			new THREE.CylinderGeometry(0.52, 0.52, 0.45, 6),
			new THREE.MeshLambertMaterial({ color: 0x9aa0a6 }));
		holder.position.y = 3.2;
		group.add(holder);
		var eraser = new THREE.Mesh(
			new THREE.CylinderGeometry(0.45, 0.45, 0.55, 12),
			new THREE.MeshLambertMaterial({ color: 0xe88ca0 }));
		eraser.position.y = 3.7;
		group.add(eraser);
		var scale = 1.5;
    ...
    // Modeli 3D i lapsit ndërtohet proceduralisht (pa file-e të jashtme — S3 origjinal i vdekur)
    buildPencil();
    scene.add( pencil );
    stopDrawing();
    onReady();
...
(function(window) {
    ...
    // Hija vizatohet si elips i thjeshtë (pa SVG të jashtëm)
		var el = new paper.Path.Ellipse({
			center: new paper.Point(0, 85),
			size: new paper.Size(150, 48)
		});
		el.fillColor = new paper.Color(0, 0, 0, 0.5);
